...
def init_db():
    ...
    conn.execute('''
      CREATE TABLE IF NOT EXISTS notes (
        ...
        user_id INTEGER,
        category_id INTEGER,
        google_event_id TEXT,   -- Вот новая колонка
...
def create_google_event(title, content, date):
...
def add():
    ...
    if request.method == 'POST':
        ...
        # Создаем событие в Google Календаре
        google_event_id = None
        try:
            if 'credentials' in session:
                google_event_id = create_google_event(title, content, date)
        except Exception as e:
            print(f"Ошибка создания события в Google: {e}")

        conn.execute('''
          INSERT INTO notes (title, content, date, category_id, user_id, google_event_id)
          VALUES (?, ?, ?, ?, ?, ?)
        ''', (title, content, date, category_id, current_user.id, google_event_id))
        conn.commit()
        conn.close()
...
def profile():
    ...
    avatar = row['avatar'] if row['avatar'] else 'default.png'
...
def sync_notes():
    ...
    conn = get_db_connection()
    notes = conn.execute(
        'SELECT id, title, content, date FROM notes WHERE user_id = ? AND date IS NOT NULL AND google_event_id IS NULL',
        (current_user.id,)
    ).fetchall()
